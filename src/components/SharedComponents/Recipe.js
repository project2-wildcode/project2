...
class Recipe extends Component {
	...
	componentDidMount() {
		const { recipeName, rating, time, level, people } = this.props.location.state;
		const url = `https://www.themealdb.com/api/json/v2/9973533/search.php?s=${recipeName}`;
		axios.get(url).then((response) => response.data.meals[0]).then((recipeData) => {
			const newInfo = {
				recipeRating: rating,
				perpTime: time,
				difficulty: level,
	...
	render() {
		...
		return (
			<div className="recipe-container">
				...
				<div className="recipe-info">Number of People:{recipeInfo.numOfPeople}</div>
				<img src={recipeInfo.strMealThumb} alt={recipeInfo.strMeal} />
				<div className="recipe-instructions">{recipeInfo.strInstructions}</div>
				<div className="ingredients">{this.filteredIngredient().map((ingredient) => <p>{ingredient}</p>)}</div>
				<div />
			</div>
		);
	}
